...
async function freeBoardLike(req, res) {
  try {
    ...
    const selectRow = await freeBoardLikeRepository.selectFreeBoardLike(postId,
        user);

    if (selectRow === undefined) {
      const insertRow = await freeBoardLikeRepository.insert(decodedToken,
          postId, null);
    ...
    if (selectRow.user_id === user.id) {
      const deleteRow = await freeBoardLikeRepository.deleteFreeBoardLike(user,
          postId);
...
async function freeBoardCommentsLike(req, res) {
  try {
    ...
    const selectRow = await freeBoardLikeRepository.selectFreeBoardCommentsLike(
        user, commentId);

    if (selectRow === undefined) {
      const insertRow = await freeBoardLikeRepository.insert(user, null,
          commentId);
    ...
    if (selectRow.user_id === user.id) {
      const deleteRow = await freeBoardLikeRepository.deleteFreeBoardCommentsLike(
          user, commentId);
